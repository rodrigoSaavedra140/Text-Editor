...
También podés abrir un archivo directamente al arrancar:

```bash
cargo run -- archivo.txt
```
...
## 🖥️ Interfaz en pantalla (Versión de Terminal)

La pantalla se divide en tres zonas:

- **Área de edición**: el texto del archivo, ocupa el resto de la pantalla.
- **Barra de ayuda** (penúltima fila): muestra en vivo los atajos de teclado disponibles según el modo en el que estés (Normal, Insert o Comando) — así no hace falta memorizar todo de entrada.
- **Barra de estado** (última fila): modo actual, nombre del archivo, si hay cambios sin guardar (`[+]`), posición del cursor, y mensajes del editor (confirmación de guardado, errores, etc.).
...
- `:` — Cambia al modo **Comando** (ver más abajo).
...
### Modo Comando (`:`)
Modo estilo Vim para guardar, abrir y salir escribiendo el nombre de archivo a mano. Se entra con `:` desde el modo Normal; mientras escribís, la barra de estado va mostrando el comando tal como lo tipeás. `Enter` lo ejecuta, `Esc` lo cancela sin hacer nada.

| Comando | Acción |
|---|---|
| `:w` | Guarda con el nombre de archivo actual. |
| `:w archivo.txt` | Guarda (o guarda como) con ese nombre. |
| `:q` | Sale del editor. |
| `:wq [archivo.txt]` | Guarda y sale en un solo paso. |
| `:e archivo.txt` | Abre ese archivo en el buffer actual. |
| `:e! archivo.txt` | Igual, pero descarta cambios sin guardar sin preguntar. |

Si intentás `:e` sin el `!` y hay cambios sin guardar en el buffer actual, el editor se niega y te avisa en la barra de estado (`hay cambios sin guardar — usá :e! para descartarlos`) — guardá primero con `:w` o usá `:e!` si de verdad querés descartarlos. Al abrir un archivo nuevo, el historial de deshacer se reinicia.
...
> **Nota:** este diagrama refleja el diseño antes de agregar la barra de ayuda y el modo Comando. La lógica central (Rope, Buffer, UndoStack) no cambió — lo nuevo son las capacidades de `Editor`/`InputHandler`/`Renderer` descritas en la sección de atajos más arriba.
